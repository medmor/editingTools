...
export class FigureExprotService {
    ...
    openOdt(): Observable<{ content: string; figures: Map<string, string> }> {
        return new Observable((observer) => {
            ...
            const request =
                this.electronService.remote.dialog.showOpenDialogSync(
                    undefined,
                    dialogOptions
                );
            if (request) {
                this.currentFilePath = this.electronService.path.dirname(
                    request[0]
                );
                this.currentFileName = this.electronService.path.basename(
                    request[0]
                );
    ...
    countFiguresSimple(node: ChildNode, figures: Map<string, string>) {
        for (const child of (node as any).childNodes) {
            if (child.nodeName === NODE_NAMES.IMAGE) {
                this.figuresCount++;
                const previousKey = this.electronService.path.basename(
                    (child as Element).getAttribute('xlink:href')
                );
    ...
    countFiguresExam(node: Element, figures: Map<string, string>) {
        for (const child of (node as any).children) {
            if (
                child.tagName === NODE_NAMES.H &&
                (child as Element).getAttribute('text:outline-level') === '1'
            ) {
                this.heading1++;
